...
from model.basic_model import similar_model
...
def main(args):
    ...
    train_cross_entropy_epochs = []
    train_accuracy_epochs = []
    valid_cross_entropy_epochs = []
    ...
    for time in range(10):
        ...
        logits = similar_model(X_placeholder, time)
        ...
        with tf.Session() as sess:
            ...
            train_writer = tf.summary.FileWriter(os.path.join(args.LOG_DIR_PATH, "nbr_para_gen_{}".format(time), "Tensorboard/train/"), sess.graph)

            sess.run(tf.global_variables_initializer())

            total_parameters = count_model_parameters()
            total_para_epochs.append(total_parameters)
            ...
            train_cross_entropy_epochs.append(train_cross_entropy_)
            train_accuracy_epochs.append(train_accuracy_)

            valid_cross_entropy_epochs.append(valid_cross_entropy_)
            valid_accuracy_epochs.append(valid_accuracy_)

            # save model
            saver = tf.train.Saver()
            save_model_dir_path = os.path.join(args.SAVE_MODLE_DIR_PATH, "nbr_para_gen_{}".format(time), "model")
    ...
    # save loss process
    train_cross_entropy_epochs = np.asarray(train_cross_entropy_epochs)
    train_loss_save_path = os.path.join(args.LOG_DIR_PATH, "nbr_para_gen", "train_loss.npy")
    ...
    valid_cross_entropy_epochs = np.asarray(valid_cross_entropy_epochs)
    valid_loss_save_path = os.path.join(args.LOG_DIR_PATH, "nbr_para_gen", "valid_loss.npy")
    ...
    train_accuracy_epochs = np.asarray(train_accuracy_epochs)
    train_accuracy_save_path = os.path.join(args.LOG_DIR_PATH, "nbr_para_gen", "train_accuracy.npy")
    ...
    valid_accuracy_epochs = np.asarray(valid_accuracy_epochs)
    valid_accuracy_save_path = os.path.join(args.LOG_DIR_PATH, "nbr_para_gen", "valid_accuracy.npy")
    ...
    total_para_epochs = np.asarray(total_para_epochs)
    total_para_save_path = os.path.join(args.LOG_DIR_PATH, "nbr_para_gen", "total_para.npy")
    if not os.path.exists(os.path.dirname(total_para_save_path)):
        os.makedirs(os.path.dirname(total_para_save_path))
    np.save(total_para_save_path, total_para_epochs)
    print("Train accuracy process to path: ", total_para_save_path)
