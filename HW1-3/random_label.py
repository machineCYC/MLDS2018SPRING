...
def main(args):
    ...
    y_train = y_train[random_order][0:train_size_data] # FIXME: random label
    ...
    predict = simple_model(X_placeholder)
    ...
    accuracy = tf.reduce_mean(
                    tf.cast(
                        tf.equal(
                            tf.argmax(Y_placeholder, dimension= 1),
                            tf.argmax(predict, dimension= 1)
                        ), 
                    tf.float32))
    ...
    with tf.Session() as sess:
        ...
        for e in range(epochs):
            ...
            # valid model
            random_choice = np.random.randint(0, len(X_valid), size=batch_size) # FIXME: need check
